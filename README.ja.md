...
- **Herdr 0.9.0 を推奨**。最低対応バージョンは **0.8.2**
- Linux または macOS
- 上記の Kitty Graphics 対応ターミナル
- Rust と Cargo（現在のインストール処理はソースからプラグインをビルドします）

2026-09-16 時点では、**クライアントとサーバーの両方が Herdr 0.9.0** の環境を主な検証対象としています。
Hop **0.1.1** は、このバージョンでポップアップのセルサイズ照会がタイムアウトする問題を修正します。
ローカルのウォーム起動測定では、ポップアップ起動からセルサイズ取得までが 171–174 ms から
23–29 ms に短縮されました。キー入力から画面表示までの全体の遅延を測定した値ではありません。

最低対応バージョンは 0.8.2 のままです。差分ゼロの resize の動作をソースで確認し、任意の更新要求が
失敗した場合のフォールバックを回帰テストで検証していますが、0.8.2 での今回のリリースの
エンドツーエンド性能は未計測です。ピッカー起動ごとにローカル socket 要求が 1 回増えます。
複数クライアントでの動作は今回未検証です。詳細は[診断と検証範囲](docs/bugs/herdr-0.9-popup-latency.md)を参照してください。
