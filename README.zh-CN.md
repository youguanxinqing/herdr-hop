...
- **推荐 Herdr 0.9.0**；最低支持版本为 **0.8.2**
- Linux 或 macOS
- 上述兼容 Kitty Graphics 的终端
- Rust 和 Cargo（当前安装过程需要从源码构建插件）

截至 2026-09-16，**客户端和服务端均为 Herdr 0.9.0** 是当前主要实测环境。
Hop **0.1.1** 修复了该版本下弹窗字符尺寸查询超时的问题。本机热启动测量中，
从打开弹窗到取得字符尺寸的耗时由 171–174 ms 降至 23–29 ms；这不是完整的按键到屏幕显示延迟。

最低兼容版本保持 0.8.2。已核对其零增量 resize 的源码行为，回归测试覆盖了可选刷新失败时的
回退路径，但尚未在 0.8.2 上实测本次发布的端到端性能。每次打开选择器会增加一次本地 socket
请求。本次发布尚未验证多客户端场景。详见[问题诊断与测试范围](docs/bugs/herdr-0.9-popup-latency.md)。
